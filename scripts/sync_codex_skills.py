...
Codex discovers repo-local skills under `.agents/skills/<name>/SKILL.md`. etch keeps
its canonical skill at `skills/etch/` (Claude-native). Symlinks are unreliable for
...
directory (`./plugins/<name>`). The `plugins/etch/` install projection is generated
from the same canonical files (plus `.codex-plugin/`) and uses real directories for
the same cross-platform reason.
...
def main() -> int:
    ...
    if args.check:
        ...
        for name in install_orphan_dirs:
            problems.append(f"plugins/etch/skills/{name} orphaned (no skills/{name})")
        if problems:
    ...
    for name in install_orphan_dirs:
        ...
        print(f"removed orphan plugins/etch/skills/{name}")
